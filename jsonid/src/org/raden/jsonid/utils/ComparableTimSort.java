 ...
 * no performance benefit over TimSort in conjunction with a comparator that
 * simply returns {@code ((Comparable)first).compareTo(Second)}. If this is the
...
public class ComparableTimSort {
	 ...
	 * set this field to true and enable them in VM with a command line flag. If
	 * you modify this class, please do test the asserts!
	 */
	...
	private ComparableTimSort(Object[] a) {
		...
		int len = a.length;
		Object[] newArray = new Object[len < 2 * INITIAL_TMP_STORAGE_LENGTH ? len >>> 1
	...
	private static int countRunAndMakeAscending(Object[] a, int lo, int hi) {
		...
		if (((Comparable<Object>) a[runHi++]).compareTo(a[lo]) < 0) { // Descending
			while (runHi < hi
					&& ((Comparable<Object>) a[runHi]).compareTo(a[runHi - 1]) < 0)
				runHi++;
	...
	private void mergeAt(int i) {
		 ...
		 */
		len2 = gallopLeft((Comparable<Object>) a[base1 + len1 - 1], a, base2,
				len2, len2 - 1);
		if (DEBUG)
			assert len2 >= 0;
		if (len2 == 0)
	 ...
	 */
	private static int gallopLeft(Comparable<Object> key, Object[] a, int base,
			int len, int hint) {
		if (DEBUG)
	...
	private void mergeLo(int base1, int len1, int base2, int len2) {
		...
		outer: while (true) {
			...
			do {
				if (DEBUG)
					assert len1 > 1 && len2 > 0;
				count1 = gallopRight((Comparable) a[cursor2], tmp, cursor1,
						len1, 0);
				if (count1 != 0) {
				...
				count2 = gallopLeft((Comparable) tmp[cursor1], a, cursor2,
						len2, 0);
				if (count2 != 0) {
		...
		outer: while (true) {
			...
			do {
				if (DEBUG)
					assert len1 > 0 && len2 > 1;
				count1 = len1
						- gallopRight((Comparable) tmp[cursor2], a, base1,
								len1, len1 - 1);
				if (count1 != 0) {
					dest -= count1;
					cursor1 -= count1;
				...
				count2 = len2
						- gallopLeft((Comparable) a[cursor1], tmp, 0, len2,
								len2 - 1);
				if (count2 != 0) {
					dest -= count2;
					cursor2 -= count2;
