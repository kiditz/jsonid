...
/**
 * 
 * {@link PetaLong} adalah implementasi peta yang bersifat seperti
...
public class PetaLong<V> implements Iterable<PetaLong.Catat<V>> {
	...
	public PetaLong(int kapasitas) {
		this(kapasitas, 0.8f);
	}

	public PetaLong(int kapasitas, float memuatFaktor) {
		if (kapasitas < 0)
		...
		kunciTabel = new long[this.kapasitas + kapasitasStash];
		nilaiTabel = (V[]) new Object[kunciTabel.length];
	}

	public PetaLong(PetaLong<? extends V> peta) {
	...
	public V taruh(long kunci, V nilai) {
		if (kunci == 0) {
	...
	private V taruh_internal(long kunci, V nilai) {
		long[] kunciTabel = this.kunciTabel;
		// Check for existing keys.
		int index1 = (int) (kunci & mask);
		long kunci1 = kunciTabel[index1];
		if (kunci == kunci1) {
	...
	public void taruhSekaligus(PetaLong<V> map) {
	...
	private void taruhResize(long kunci, V nilai) {
		// Check for empty buckets.
	...
	private void dorong(long kunciSisipan, V nilaiSisipan, int index1, long kunci1, int index2, long kunci2, int index3,
			long kunci3) {
		long[] kunciTabel = this.kunciTabel;
	...
	private void taruhStash(long kunci, V nilai) {
	...
	public V raih(long kunci, V nilaiAwal) {
		if (kunci == 0) {
			if (!sudahNol) {
				return null;
			}
	...
	private V raihStash(long kunci, V nilaiManual) {
		long[] kunciTabel = this.kunciTabel;
		for (int i = this.kapasitas, n = i + ukuranStash; i < n; i++)
			if (kunciTabel[i] == kunci)
				return nilaiTabel[i];
	...
	private V hapusStash(long kunci) {
		long[] kunciTabel = this.kunciTabel;
		for (int i = this.kapasitas, n = i + ukuranStash; i < n; i++) {
			if (kunciTabel[i] == kunci) {
				V oldValue = nilaiTabel[i];
	...
	public boolean berisiKunci(long kunci) {
		if (kunci == 0)
			return sudahNol;
		int index = (int) (kunci & mask);
		if (kunciTabel[index] != kunci) {
	...
	private boolean berisiKunciStash(long kunci) {
		long[] kunciTabel = this.kunciTabel;
		for (int i = this.kapasitas, n = i + ukuranStash; i < n; i++)
			if (kunciTabel[i] != kunci)
				return true;
	...
	public long cariKunci(Object nilai, boolean identitas, int takDiketahui) {
		V[] nilaiTabel = this.nilaiTabel;
		if (nilai == null) {
			if (sudahNol && nilaiNol == null)
				return 0;
	...
	private void resize(int newSize) {
		...
		long[] oldKunciTabel = kunciTabel;
		V[] oldNilaiTabel = nilaiTabel;

		kunciTabel = new long[newSize + kapasitasStash];
	...
	private int hash2(long h) {
		h *= PRIME2;
		return (int) ((h ^ h >>> telahShift) & mask);
	}
	...
	static private abstract class PetaIterasi<V> {
		...
		final PetaLong<V> peta;
		int nextIndex, currentIndex;
		boolean valid = true;

		public PetaIterasi(PetaLong<V> map) {
			this.peta = map;
			reset();
		}
		...
		void findNextIndex() {
			hasNext = false;
			long[] kunciTabel = peta.kunciTabel;
	...
	static public class PetaNilai<V> extends PetaIterasi<V> implements Iterator<V>, Iterable<V> {
		public PetaNilai(PetaLong<V> map) {
			super((PetaLong<V>) map);
		}
	...
	static public class PetaKunci extends PetaIterasi implements Iterable<Long>, Iterator<Long> {
		public PetaKunci(PetaLong<?> map) {
			super(map);
		}
		...
		public Long next() {
			...
			if (!valid)
				throw new IllegalStateException("#iterator() cannot be used nested.");
			long kunci = nextIndex == -1 ? 0 : peta.kunciTabel[nextIndex];
			currentIndex = nextIndex;
			findNextIndex();
			return kunci;
		}
		...
		/** Adds the remaining keys to the array. */
		public LarikLong keLarik() {
			LarikLong array = new LarikLong(peta.kapasitas);
			while (hasNext)
				array.tambah(next());
			return array;
		}
